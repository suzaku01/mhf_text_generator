...
func (s *Session) makeSignInResp(uid int) []byte {
	...
	bf.WriteUint8(2)                                   // file/patch server count
	...
	bf.WriteUint8(1)           // notice_count
	bf.WriteUint16(0) 
	data, _ := hex.DecodeString("OutputDataHere")
	bf.WriteBytes(data)			//notification message
	...
	bf.WriteUint16(1)
	bf.WriteUint16(20000)
	uint16PascalString(bf, "127.0.0.1:8080")
